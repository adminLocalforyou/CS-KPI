...
const PublicAnswers: React.FC<PublicAnswersProps> = ({ assessments, submissions }) => {
  ...
  // Anonymization Map (Consistent per session or just by index)
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
           ...
           <div className="bg-white rounded-[3.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Score Breakdown</p>
                    ...
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 md:col-span-2">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Learning Note</p>
                       <p className="text-sm font-bold text-slate-600 italic">"Review the correct answers below to understand the reasoning for this topic."</p>
                    </div>
                 </div>
                          ...
                          <div key={q.id} className="p-8 rounded-[2rem] bg-slate-50 border border-slate-100 space-y-4">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                                <div className="space-y-2">
                                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Employee Response</p>
                                   <div className={`p-4 rounded-2xl text-sm font-bold ${isCorrect ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                                      {staffAnswer || '(No response recorded)'}
                                   </div>
                                </div>
                                {q.type === 'choice' && (
                                   <div className="space-y-2">
                                      <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Correct Answer (Key)</p>
                                      <div className="p-4 bg-slate-900 rounded-2xl text-sm font-black text-white">
                                         {q.correctAnswer}
                                      </div>
                                   </div>
                                )}
                 ...
                 {viewingSubmission.managerFeedback && (
                    <div className="p-8 bg-amber-50 rounded-[2.5rem] border border-amber-100 flex items-start gap-4">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">General Feedback</p>
              ...
              <div className="p-8 bg-slate-50 border-t border-slate-100 text-center">
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center justify-center gap-2">
                    <Shield size={14} /> Anonymized Public Answer Key System
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-blue-100 text-lg mt-1 font-medium italic">Learn from the latest performance evaluations (Anonymized Data)</p>
            </div>
          </div>
          <div className="bg-white/10 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 text-center min-w-[200px]">
             <p className="text-[10px] font-black text-blue-200 uppercase tracking-widest mb-1">Total Graded</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {filteredSubmissions.length === 0 ? (
            <div className="col-span-full py-20 bg-white rounded-[3.5rem] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-300 space-y-4">
               <FileText size={64} />
               <p className="font-black uppercase tracking-widest">No graded submissions available yet</p>
            </div>
         ) : (
            filteredSubmissions.map((s, idx) => (
               ...
               >
                  ...
                  <div className="pt-6 border-t border-slate-50 flex items-center justify-between group-hover:text-blue-600 transition-colors">
                     <span className="text-[10px] font-black uppercase tracking-widest">View Answer Comparison</span>
                     <ArrowRight size={18} />
                  </div>
               </div>
            ))
      ...
      <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 flex items-start gap-6">
         ...
         <div className="space-y-1">
            <h5 className="font-black text-slate-800 text-sm uppercase">Privacy & Learning Notice</h5>
            <p className="text-xs text-slate-500 font-medium leading-relaxed">
               This space is for peer learning. Employee names are hidden to maintain privacy. Use this as a guide to compare your own logic with the standard answer key provided by the supervisor.
            </p>
         </div>
      </div>
    </div>
